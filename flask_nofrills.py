...
def flask_nofrills(user_search_term):
    ...
    driver.get(
        "https://www.nofrills.ca/search?search-bar={}".format(user_search_term))
    ...
    driver.find_element_by_css_selector(
        "#site-layout > div.modal-dialog.modal-dialog--region-selector > div.modal-dialog__content > div > div > ul > li:nth-child(2) > button").click()
    ...
    for result in search_results:
        ...
        product_name = result.find(
            "span", {"class": "product-name__item"}).text.strip()
        product_price = result.find(
            "span", {"class": "price__value"}).text.strip()
        product_unit = result.find(
            "span", {"class": "price__unit"}).text.strip()
        product_link = "https://www.nofrills.ca" + \
            result.find(
