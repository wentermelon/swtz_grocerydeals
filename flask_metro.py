...
def flask_metro(user_search_term):
    URL = 'https://www.metro.ca/en/online-grocery/search?sortOrder=relevance&filter={}&freeText=true'.format(
        user_search_term)
    ...
    products = results_2.find('div', {
                              "class": 'products-tiles-list products-tiles-list--search searchOnlineResults'})
    ...
    for product in products_list:
        ...
        product_price = product.find(
            "span", {"class": "pi-price price-update"}).text.strip()
        ...
        product_link = product.find(
            'a', {"class": "pt--image product-details-link"})['href'].strip()
    ...
    # Passing as dict makes more sense as it's easier to parse from flask
